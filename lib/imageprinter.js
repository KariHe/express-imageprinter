 ...
 *
 * @type {{size: {width: number, height: number}, crop: { enabled: boolean }, quality: number}}
 */
...
module.exports.middleware = function( options ) {
    ...
    var static = express.static( _config.destination, {
...
function processImage( request, next ) {
    ...
    mkdirp( path.dirname( request.cacheFile ), function(err) {
        ...
        imageResizeAndCrop( img, request.options );
 ...
 * @returns {*}
 */
function imageResizeAndCrop( img, options ) {
    console.log( options );
    var crop = {
        width: options.width,
